...
function BoardCommittees({ committees, heading = "Get to know our team" }: BoardCommitteesProps) {
  ...
  return (
    <div className="flex w-full flex-col items-center">
      <h1 className="mb-4 text-3xl font-joyful tracking-tighter md:text-4xl">{heading}</h1>
          ...
          <div className="flex max-w-5xl flex-wrap justify-center gap-3 px-2">
            {committees.map((committee, index) => (
              <button
                ...
                className={`
                  rounded-full border px-5 py-2 text-lg font-joyful transition-all duration-300
                  ${
            ...
            {currentCommittee?.members.map((member, idx) => (
              <div key={`${member.name}-${idx}`} className="flex w-60 flex-col items-center sm:items-start">
                ...
                <div className="w-full px-1">
                  <h3 className="text-center text-2xl font-joyful leading-tight sm:text-left">{member.name}</h3>
                  <p className="text-center font-joyful text-m text-gray-500 sm:text-left">{member.role}</p>
                </div>
              </div>
            ))}
          </div>
